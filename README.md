...
- **Node.js 20+** and **git**
- A **[Supabase](https://supabase.com)** project (you'll need the project ref and a database connection string)
...
Doctor is a safe, read-only checklist command. It verifies local env values, Supabase project targeting, Supabase Auth/REST reachability, service-role reachability, deployed Google OAuth secret status, token-storage readiness, Google client-secret validity, and the Google OAuth redirect generated by the live edge function. It also prints dashboard-only manual items, including Supabase Auth URLs, Google authorized redirect URI, and Google test-user setup.
